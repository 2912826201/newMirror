...
/// 直播详情页
class LiveDetailPage1 extends StatefulWidget {
  const LiveDetailPage1({Key key, this.heroTag,this.commentDtoModel,this.fatherComment, this.liveCourseId, this
      .liveModel, this
  ...
  @override
  createState() {
    return LiveDetailPage1State(heroTag: heroTag, liveCourseId: liveCourseId, liveModel: liveModel,commentDtoModel:
    commentDtoModel,fatherComment: fatherComment);
  }
}

class LiveDetailPage1State extends State<LiveDetailPage1> {
  LiveDetailPage1State({Key key,
    this.heroTag,
    this.liveCourseId,
    this.liveModel,
...
//互动通知列表带过来的评论内容
  ...
  //外层评论ids
  String bigCommentIds="";
  //子评论ids
  String subCommentIds="";
    ...
    if (isHotOrTime) {
      if (courseCommentHot == null) {
        Map<String, dynamic> commentModel = await queryListByHot2(
            targetId: liveCourseId, targetType: 1, lastId: courseCommentHot?.lastId??null,ids: bigCommentIds, size: courseCommentPageSize);
        if (commentModel != null) {
