...
class TikTokScaffold extends StatefulWidget {
  ...
  // 相册页滑动回调
  final Function() cameraPageSlideCallBack;
  // 当前页面滑动回调
  final Function() middleSlideCallBack;
  const TikTokScaffold({
    ...
    this.cameraPageSlideCallBack,
    this.middleSlideCallBack,
    this.controller,
  }) : super(key: key);
...
class _TikTokScaffoldState extends State<TikTokScaffold>
  ...
  // 水平方向滑动中
  void onHorizontalDragUpdate(DragUpdateDetails details, screenWidth) {
    if (_initialSwipeOffset != null) {
      final offsetDifference = _initialSwipeOffset.dx - details.globalPosition.dx;
      // 在主页右滑时
      if(offsetDifference > 0 && !isCameraPage){
        return;
      }
    }
    if (!widget.enableGesture) return;
  ...
  onHorizontalDragEnd(details, screenWidth) {
    print(" // 水平方向滑动每秒滑动距离 ::: ${details.velocity.pixelsPerSecond.dx}");
    if (!widget.enableGesture  ) return;
