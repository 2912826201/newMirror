  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < dataArr.length; i++) {
      ...
      var text = TextPainter(
          ...
          text: TextSpan(
              text: '${data['num']}:\n',
              style: TextStyle(fontSize: font, color:Colors.primaries[i % Colors.primaries.length].withOpacity(opacity) ),
              children: <TextSpan>[
                TextSpan(
                    text: data['name'],
                    style:
                    TextStyle(fontSize:font, color: AppColor.textPrimary1.withOpacity(opacity)),
      ...
      // paintStyle.color = point.color.withOpacity(opacity);
    paintStyle.maskFilter = MaskFilter.blur(BlurStyle.inner,150);
      // canvas.drawCircle(Offset(point.x, point.y), r, paintStyle);
      //  正方形
