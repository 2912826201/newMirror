...
class _PhoneBindingPageState extends State<PhoneBindingPage> {
  ...
  _sendSms() {
    Navigator.of(context).push(MaterialPageRoute(builder: (context) {
      ...
      return SmsCodePage(
        phoneNumber: this.inputController.text,
      );
    }));
  }
  ...
  bool validationJudge() {
    //FIXME:等待更改
    if (inputController.text.length > 14) {
      return true;
    }
    return false;
  }
  ...
  Widget _stateMentArea() {
    //文字包为一个整体
    var bag = Padding(
      padding: EdgeInsets.only(left: 41, right: 41, top: 42.5),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          //文字的上下间距
  ...
  Widget _buttonArea() {
    ...
    var smsBtn = FlatButton(
      ...
      shape: btnStyle,
      onPressed: _sendSms,
      child: Text(_smsButtonTitle, style: _sendSmsBtnTitleStyle),
      color: _sendSmsBtnColor,
    );
