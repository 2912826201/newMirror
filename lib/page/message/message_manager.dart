...
//发送图片视频---一般不用
Future<Message> postMessageManagerImgOrVideo1(String targetId,
    bool isImgOrVideo, MediaFileModel mediaFileModel,
...
Future<Message> postMessageManagerImgOrVideo(String targetId, bool isImgOrVideo,
  ...
  Map<String, dynamic> feedMap = Map();
  feedMap["type"] =
  isImgOrVideo ? ChatTypeModel.MESSAGE_TYPE_IMAGE : ChatTypeModel
      .MESSAGE_TYPE_VIDEO;
...
Future<Message> postMessageManagerVoice(String targetId,
    ChatVoiceModel chatVoiceModel) async {
...
//发送消息
Future<Message> postMessageManager1(String targetId,
...
void postImgOrVideo(List<ChatDataModel> modelList, String targetId, String type,
    VoidCallback voidCallback) async {
  List<UploadResultModel> uploadResultModelList = await onPostImgOrVideo(
      modelList, type);
  for (int i = 0; i < modelList.length; i++) {
    ...
    if (uploadResultModelIndex >= 0) {
      modelList[i].msg = await postMessageManagerImgOrVideo(
          targetId, type == mediaTypeKeyImage, modelList[i].mediaFileModel,
...
//上传图片和视频
Future<List<UploadResultModel>> onPostImgOrVideo(List<ChatDataModel> modelList,
    String type) async {
  List<File> fileList = [];
