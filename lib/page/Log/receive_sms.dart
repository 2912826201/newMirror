...
class ReceiveSmsPage extends StatefulWidget{
  final String phoneNumber;
  ReceiveSmsPage({@required this.phoneNumber,Key key}):super(key: key);
  @override
  State<StatefulWidget> createState() {
    return _ReceiveSmsPageState(this.phoneNumber);
  }
}
class _ReceiveSmsPageState extends LogBasePageState{
  ...
  _ReceiveSmsPageState(this.phoneNumber);
 ///说明区域
  Widget _stateMentArea(){
    var bag ;
    var mainTitle = Text("输入验证码",style: TextStyle(
    ...
    var stars = "****";
    var subTitle =  Text("短信验证码已发送至 +86 "+prefixString+stars+suffixString,
      style: TextStyle(
          fontFamily: "PingFangSC",
          color: Color.fromRGBO(153, 153, 153, 1),
          fontSize: 14,
          decoration: TextDecoration.none),);
    var area1 = Container(child: mainTitle,
      margin: EdgeInsets.only(bottom: 9),);
    var area2 = Container(child: subTitle,);
    var column = Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        area1,area2
...
class SmsCounterWidget extends StatefulWidget{
  ...
  final int seconds;
  SmsCounterWidget(this.seconds,this.requestTask,{Key key}):assert(seconds!=null),super(key: key);
  @override
  State<StatefulWidget> createState() {
...
class  _SmsCounterWidgetState extends State<SmsCounterWidget>{
  ...
  Text _countText(){
    return Text('${_seconds}S',
      style: TextStyle(color: Color.fromRGBO(204, 204, 204, 1),
          fontSize: 13,
          fontFamily: "PingFangSC",decoration: TextDecoration.none),);
  }
  ...
  Widget _creatCountingWidget(){
    return  Container(
      child: FlatButton(child: _countText(),
        onPressed: null,
        padding: EdgeInsets.all(0),),
