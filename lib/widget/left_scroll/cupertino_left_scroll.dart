...
import 'left_scroll_list_view.dart';
...
class CupertinoLeftScroll extends StatefulWidget {
  ...
  BounceStyle get _bounceStyle => bounceStyle ?? (bounce ? BounceStyle() : BounceStyle.disable());
...
class CupertinoLeftScrollState extends State<CupertinoLeftScroll> with TickerProviderStateMixin {
  /// 手指滑动的坐标值
  double translateX = 0;
  ...
  double get bounceTranslate {
    ...
    return min(0, resultTranslate.clamp(-maxDragDistance - widget._bounceStyle.maxDistance, 0.0));
  }
  ...
  Map<LeftScrollCloseTag, Map<Key, LeftScrollStatus>> get globalMap => LeftScrollGlobalListener.instance.map;
  ...
  Widget getResetOpen() {
    return Consumer<SelectPositionNotifier>(
      builder: (context, notifier, child) {
        if (context.watch<SelectPositionNotifier>().singleDeletePosition == widget.itemIndex) {
          context.watch<SelectPositionNotifier>().singleDeletePosition = -1;
          open(v: 10, isOnOpen: false);
        }
        return Container();
      },
    );
  ...
  void onHorizontalDragStart(DragStartDetails details) {
    if (widget.closeTag == null) return;
    if (_ct.value == false) {
      LeftScrollGlobalListener.instance.needCloseOtherRowOfTag(widget.closeTag, widget.key);
    }
  }
  ...
  void onHorizontalDragEnd(DragEndDetails details) {
    ...
    if (details.velocity.pixelsPerSecond.dx > 200) {
      ...
      open(v: details.velocity.pixelsPerSecond.dx);
    } else {
...
class _WxStyleButtonGroup extends StatelessWidget {
  ...
  double get eachWidthOffset => (buttonWidth * children.length + bounceDistance) / children.length * progress;
