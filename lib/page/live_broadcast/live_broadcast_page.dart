...
class LiveBroadcastPageState extends State<LiveBroadcastPage> {
  ...
  Widget _getTopCalendar() {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      calendarWidgetArray.add(GestureDetector(
        child: Container(
  ...
  Widget _getItemLeftImageUi(
      LiveBroadcastModel value, int imageWidth, int imageHeight) {
    return Container(
...
//设置数据
        ...
        if (judge == 0) {
          for (int j = 0; j < liveBroadcastNewData.length; j++) {
            ...
            if (liveBroadcastNewData[j].title == calendarEvents[i].title &&
                liveBroadcastNewData[j].coachName ==
                    calendarEvents[i].description) {
              judge = 1;
  ...
  void createEvent(String id, DeviceCalendarPlugin _deviceCalendarPlugin,
      LiveBroadcastModel value) async {
