...
/// [VideoVisibilityDetector] will be invoked at most once per
/// [VisibilityDetectorController.updateInterval] (unless forced by
/// [VisibilityDetectorController.notifyNow]).  Callbacks for *all*
/// [VideoVisibilityDetector] widgets are fired together synchronously between
/// frames.

class VideoVisibilityDetector extends SingleChildRenderObjectWidget {
  /// Constructor.
  ///
  /// `key` is required to properly identify this widget; it must be unique
  /// among all [VideoVisibilityDetector] and [SliverVisibilityDetector] widgets.
  ///
  /// `onVisibilityChanged` may be `null` to disable this [VisibilityDetector].
   VideoVisibilityDetector({
    @required Key key,
...
typedef VisibilityChangedCallback = void Function(VideoVisibilityInfo info);
...
class VideoVisibilityInfo {
  ...
  ///
  /// If `size` or `visibleBounds` are omitted or null, the [VideoVisibilityInfo]
  ...
  const VideoVisibilityInfo({@required this.key, Size size, Rect visibleBounds})
      : assert(key != null),
        size = size ?? Size.zero,
        visibleBounds = visibleBounds ?? Rect.zero;

  /// Constructs a [VideoVisibilityInfo] from widget bounds and a corresponding
  /// clipping rectangle.
  ///
  /// [widgetBounds] and [clipRect] are expected to be in the same coordinate
  /// system.
  factory VideoVisibilityInfo.fromRects({
    @required Key key,
  ...
  /// Returns true if the specified [VideoVisibilityInfo] object has equivalent
  /// visibility to this one.
  bool matchesVisibility(VideoVisibilityInfo info) {
    // We don't override `operator ==` so that object equality can be separate
    // from whether two [VideoVisibilityInfo] objects are sufficiently similar
  ...
  @override
  String toString() {
    return 'VideoVisibilityInfo(size: $size visibleBounds: $visibleBounds)';
  }
}
...
class VisibilityDetectorLayer extends ContainerLayer {
  ...
  void _scheduleUpdate() {
    ...
    if (updateInterval == Duration.zero) {
      ...
      // middle of a frame.
      print("_timer111111111111:::$_timer");
      if (isFirstUpdate) {
        ...
        print("_timer222222222222222:::$_timer");
      ...
      print("_timer33333333333333:::$_timer");
    } else {
      print("_timer444444444444444:::$_timer");
  ...
  /// frame.
    ...
    SchedulerBinding.instance
        .scheduleTask<void>(_processCallbacks, Priority.animation);
  }
  ...
  static void _processCallbacks() {
    for (final layer in _updated.values) {
      if (!layer.attached) {
        layer._fireCallback(VideoVisibilityInfo(
      ...
      final info = VideoVisibilityInfo.fromRects(
          key: layer.key,
  ...
  /// Invokes the visibility callback if [VideoVisibilityInfo] hasn't meaningfully
