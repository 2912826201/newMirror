...
class FileUtil {
  ...
  // 当上传失败时返回的是null
  Future<UploadResults> _upload(List<File> fileList, int type,
      Function(double percent) progressCallback) async {
    ...
    syStorage.onChanged().listen((dynamic percent) {
      ...
      print("单文件进度））））））））））））））$percent");
      double totalPercent =
          (1.0 * _finishedCount + p) / (1.0 * fileList.length);
    ...
    for (int i = 0; i < fileList.length; i++) {
      ...
      // 上传文件
      UploadResult result = await syStorage.upload(
          fileList[i].path, token.upToken, key);
      // print("&@@@@@@@@@@@@@@${file.path}");
      // print(result);
  ...
  Future<UploadResults> uploadFiles(List<File> fileList,
      Function(double percent) progressCallback) {
    return _upload(fileList, 0, progressCallback);
  }

  Future<UploadResults> uploadMedias(List<File> fileList,
      Function(double percent) progressCallback) {
    return _upload(fileList, 1, progressCallback);
  }

  Future<UploadResults> uploadPics(List<File> fileList,
      Function(double percent) progressCallback) {
  ...
  String _genKey(File file) {
    ...
    return "ifapp/${Application.token.uid}/" + DateTime
        .now()
  ...
  Future<File> writeImageDataToFile(Uint8List imageData,
